...
export const InfoList = ({ label, input, button }) => {
  const { name, value, type, required, onChange, placeholder, checkInput } =
    input;
  const isError =
    value.length !== 0 && checkInput && checkInput.isConfirm === false;
  ...
  return (
    <section className={style.inputList}>
      ...
      <section className={style.inputList_input}>
        ...
        />
        {isError && (
          <span className={style.errorMsg}>{checkInput.errorMessage}</span>
        )}
      </section>
